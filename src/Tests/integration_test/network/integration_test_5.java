...
public class integration_test_5 implements Runnable {
	...
	public void setUp() throws Exception {
    	...
    	//init
        ConnectionHandler connectionHandler = ConnectionHandler.connect_server("127.0.0.1" , (short) 6665); 
	...
	public void test() {	
		...
		assertTrue(cl_handler.changePolicy(p2));
		assertTrue(cl_handler.logout());

		
		// delete replay post 
		Vector<IPost> member1_ReplyPost =  cl_handler.show_ReplyPost();
		if(member1_ReplyPost.size() > 0)
			assertTrue(cl_handler.deletePost(member1_ReplyPost.get(0)));



		// delete Thread post 
		Vector<IPost> member1_ThreadPost =  cl_handler.show_TreadPost();
		if(member1_ThreadPost.size() > 0)
			assertTrue(cl_handler.deletePost(member1_ThreadPost.get(0)));		
		
		 member1_ThreadPost =  cl_handler.show_TreadPost();
